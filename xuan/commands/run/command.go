...
func RestartServices(root string, doAPI, doRPC bool) error { return restartServices(root, doAPI, doRPC) }

// --- internal implementation (migrated from old main) ---

func runAll() error {
	fmt.Println("重启 docker-compose 服务...")
	if err := dockerComposeDownUp(); err != nil { return err }
	...
	if err := startEtcd(); err != nil { return err }
	...
	for _, p := range targets {
		if err := restartPath(p); err != nil { return err }
	}
	fmt.Println("全部启动完成")
	return nil
}
...
func dockerComposeDownUp() error {
	if err := runCmdStreaming("docker-compose", "down"); err != nil { return err }
...
func restartServices(root string, doAPI, doRPC bool) error {
	if doRPC { if err := restartPath(filepath.Join(root, "rpc")); err != nil { return err } }
	if doAPI { if err := restartPath(filepath.Join(root, "api")); err != nil { return err } }
	return nil
}

func stopServices(root string, doAPI, doRPC bool) error {
	if doRPC { _ = stopPath(filepath.Join(root, "rpc")) }
	if doAPI { _ = stopPath(filepath.Join(root, "api")) }
	return nil
}
...
func buildAndRunService(servicePath string) error {
	if _, err := os.Stat(servicePath); os.IsNotExist(err) { return fmt.Errorf("目录不存在: %s", servicePath) }
	serviceName := deriveServiceName(servicePath)
	fmt.Printf("构建并启动: %s (%s)\n", serviceName, servicePath)
	if err := runCmdInDirStreaming(servicePath, "go", "build", "-o", serviceName, "."); err != nil { return err }
	logFile := filepath.Join(filepath.Dir(servicePath), serviceName+".log")
	binPath := filepath.Join(servicePath, serviceName)
	f, err := os.OpenFile(logFile, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, 0644)
	if err != nil { return fmt.Errorf("打开日志文件失败: %v", err) }
	cmd := exec.Command(binPath)
	cmd.Stdout = f
	cmd.Stderr = f
	if err := cmd.Start(); err != nil { _ = f.Close(); return fmt.Errorf("启动进程失败: %v", err) }
	_ = f.Close()
	return nil
}
...
func startEtcd() error {
	logFile := "etcd.log"
	f, err := os.OpenFile(logFile, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, 0644)
	if err != nil { return fmt.Errorf("打开 etcd 日志失败: %v", err) }
	cmd := exec.Command("etcd", "--data-dir=./etcd-data")
	cmd.Stdout = f
	cmd.Stderr = f
	if err := cmd.Start(); err != nil { _ = f.Close(); return fmt.Errorf("启动 etcd 失败: %v", err) }
	_ = f.Close()
	return nil
}
